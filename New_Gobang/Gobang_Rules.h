...
int Gobang_Rules::Eveluate()
{
	int x, y, k;
	int Side;
	memset(TypeRecord, TOBEANALSIS, 15 * 15 * 4 * 4);
...
int Gobang_Rules::Analysis(int x, int y)
{
	int i, j, len;
	string side1, side2;
	int Color = COM;
	for (i = 0; i < 4; i++)
	{
		if (TypeRecord[x][y][i] == TOBEANALSIS)
		{
			len = 0;
			side1.clear(); side2.clear();
			while (Gobang::getBoard(x, y) == Color)
			{
				x -= Direct[i][0];
				y -= Direct[i][1];
			}
			for (j = 0; j < 5; j++, x -= Direct[i][0], y -= Direct[i][1]) {
				if (Gobang::getBoard(x, y) == EMPTY && x >= 0 && y >= 0)
					side1 += '#';
				else if (Gobang::getBoard(x, y) == Color && x >= 0 && y >= 0)
					side1 += '0';
				else
					side1 += '1';
			}
			x += Direct[i][0] * 6;
			y += Direct[i][1] * 6;
			while (Gobang::getBoard(x, y) == Color && x >= 0 && y >= 0)
			{
				len++;
				TypeRecord[x][y][i] = ANALSISED;
				x += Direct[i][0];
				y += Direct[i][1];
			}
			for (j = 0; j < 5; j++, x += Direct[i][0], y += Direct[i][1]) {
