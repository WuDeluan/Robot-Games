...
public:
	...
	int NullMoveSearch(int depth, int alpha, int beta, int palyer);
	void UpdateKillers(int depth, Point move); //更新杀手启发表
	int NegaScout_hash_history_killer(int depth, int alpha, int beta, int player);
	int PVS(int depth, int alpha, int beta, int player);
	int Search(int depth, int alpha, int beta, int palyer);
};
...
void Gobang_Rules::SpecificOpening()
{
	...
	{
		_setBoard(6, H, WHITE); //白2位于(6, H)
		_setBoard(15 - Lib[k].x, Lib[k].y, BLACK); //找到对应于棋盘的位置，落子
	}
	else if (k >= 13 && k < 26) //后13种为斜指开局
	{
		_setBoard(6, I, WHITE); //白2位于(6, I)
		_setBoard(15 - Lib[k].x, Lib[k].y, BLACK); //找到对应于棋盘的位置，落子 
	}
}
...
int Gobang_Rules::CreateMoveList(int depth)
{
	...
	{
		MoveList[depth][MoveCount++] = Killers[depth][0];
		if (Killers[depth][1].score != -1)
			MoveList[depth][MoveCount++] = Killers[depth][1];
	}
...
void Gobang_Rules::SortMoveList(int depth, int MoveCount)
{
	...
	for (i = 0; i < MoveCount; i++)
	{
		if (MoveList[depth][i].move.x == Killers[depth][0].move.x && MoveList[depth][i].move.y == Killers[depth][0].move.y)
			MoveList[depth][i].score = +INFINITY;
		else if (MoveList[depth][i].move.x == Killers[depth][1].move.x && MoveList[depth][i].move.y == Killers[depth][1].move.y)
			MoveList[depth][i].score = +INFINITY - 1; 
		else
...
void Gobang_Rules::UpdateKillers(int depth, Point move)
{
...
int Gobang_Rules::NegaScout_hash_history_killer(int depth, int alpha, int beta, int player)
{
	static int max_depth = depth;
	int i, value, MoveCount;
	Point bestMove;
	...
	{
		MakeMove(MoveList[depth][i].move, player);
		value = -NegaScout_hash_history_killer(depth - 1, -beta, -alpha, notplayer(player));
		MakeNullMove(MoveList[depth][i].move);
		if (value >= beta)
		{
			RecordHash(depth, value, hash_BETA, MoveList[depth][i].move);
			UpdateKillers(depth, MoveList[depth][i].move);
			AddHistoryScore(depth, MoveList[depth][i].move);
			return beta;
		}
		if (value > alpha)
...
int Gobang_Rules::NullMoveSearch(int depth, int alpha, int beta, int player)
{
	static int max_depth = depth;
	int i, value, MoveCount;
	static int istest = 0;
	...
	if (depth >= 3 && istest == 0)
	{
		...
		for (i = 0; i < MoveCount; i++)
		{
			MakeMove(MoveList[depth][i].move, notplayer(player));
			value = -NegaScoutSearch(depth - 1 - R, -beta, -alpha, notplayer(player));
			MakeNullMove(MoveList[depth][i].move);
...
int Gobang_Rules::Search(int depth, int alpha, int beta, int player)
{
	static int max_depth = depth;
	static int istest = 0;
	int i, value, MoveCount;
	...
	for (i = 0; i < MoveCount; i++)
	{
		...
		if (foundPV)
		{
			value = -Search(depth - 1, -alpha - 1, -alpha, notplayer(player));
			if ((value > alpha) && (value < beta))
			{ // 检查失败
				value = -Search(depth - 1, -beta, -value, notplayer(player));
			}
		}
		else
			value = -Search(depth - 1, -beta, -alpha, notplayer(player));
